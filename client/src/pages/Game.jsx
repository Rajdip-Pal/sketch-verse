...
export default function Game() {
    ...
    const buildAvatars = (images, size) => {
        let avatars = images.map((image, index) => (
            <button
                ...
                onClick={() => {
                    document.querySelectorAll(".avatars").forEach((avatar) =>
                        avatar.classList.replace("border-lime-500", "border-transparent")
                    );
                    document.getElementById(`avatar-${index}`).classList.replace(
                        "border-transparent",
                        "border-lime-500"
                    );
                    setAvatar(image);
            ...
            >
                <img
                    id={`avatar-${index}`}
                    className="avatars m-1 border-4 border-transparent rounded-xl"
                    src={image}
                    alt="Avatar"
                    width={100}
                    height={100}
    ...
    return (
        <React.Fragment>
            ...
            <motion.div
                className={`flex justify-center items-center h-[80%] w-full transition-all ${showModal ? "blur-sm" : ""
                    }`}
            ...
            >
                ...
                >
                    <h3 className="text-center text-lime-500 font-bold text-3xl mt-5">
                        Choose Avatar :
                    </h3>
                    <div className="flex justify-center align-middle items-center mt-3 md-5">
                        <div className="flex flex-col justify-center w-[80%]">
                            {buildAvatars(images, 7)}
                        </div>
                    </div>
                    ...
                    <div className="flex justify-center my-5">
                        <button
                            ...
                            className="mb-5 px-8 py-2 font-kota text-xl rounded-3xl bg-lime-500 text-black hover:bg-green-700 hover:text-white active:scale-90"
                        >
                            Join Game
                        </button>
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white p-6 rounded-xl shadow-xl flex flex-col items-center">
                        ...
                        <div className="flex justify-center mt-4">
                            <button
                                onClick={joinGame}
                                className="px-6 py-2 mr-3 bg-lime-500 text-black font-bold rounded-xl hover:bg-green-700 hover:text-white"
                            >
                                Join
                            </button>
                            <button
                                onClick={() => setShowModal(false)}
                                className="px-6 py-2 bg-gray-300 text-black font-bold rounded-xl hover:bg-gray-400"
                            >
                                Cancel
                            </button>
                        </div>
